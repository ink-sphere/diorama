...
- `DIORAMA_LOADER_MODEL_ID` — optional; overrides the litellm model id `EbookLoaderAgent` runs with (resolved per run by `resolve_agent_runtime()` in `diorama/backend/settings.py`). Defaults to `openrouter/openai/gpt-4o-mini`, which is noticeably less reliable at structure extraction than `openrouter/openai/gpt-4o` — set this to trade cost for reliability. A `gemini/...` id here routes the loader through Google AI Studio instead, and picks up `GEMINI_API_KEY` rather than `OPENROUTER_API_KEY`.
